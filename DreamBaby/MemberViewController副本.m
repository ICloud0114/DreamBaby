...
//            streetField.textAlignment = NSTextAlignmentRight;
        ...
        if (userField.text.length == 0 || nickNameField.text.length == 0 || mobileField.text == 0 || birthField.text.length == 0 ||cityField.text.length == 0|| areaField.text.length == 0 || streetField.text.length == 0) {
...
//                            streetArr = [lisDic objectForKey:@"list"];
                            [streetArr setArray:[lisDic objectForKey:@"list"]];
                        }
                    }
                }
            }
...
- (void)updateUserInfo
{
    ...
    dispatch_async(network, ^{
        ...
        NSString *rows = streetField.text;
        NSString *address = member.address;
        NSString *idCard = member.id_card;
        
        NSDictionary *param = [NSDictionary dictionaryWithObjectsAndKeys:user_id,@"user_id",user_name,@"user_name",nick_name,@"nick_name",mobile,@"mobile",email,@"email",birth,@"birthday",cityID,@"census_register",areaID,@"area",streetID,@"rows",address,@"address",idCard,@"id_card",nil];
...
{
    if (textField == cityField ||textField == streetField || textField == areaField)
    {
        CGRect rect = memberTable.frame;
        rect.origin.y = 0;
        memberTable.frame = rect;
...
-(NSString*)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component
{
    ...
    {
        NSLog(@"street---->%d",row);
        return [streetArr[row] objectForKey:@"title"];
...
-(void)selectArea
{
    ...
    if ([[areaArr[row] objectForKey:@"title"] isKindOfClass:[NSString class]])
    {
        if (![[areaArr [row] objectForKey:@"title"]isEqualToString:areaField.text])
        {
            if (streetArr.count)
            {
                [streetArr removeAllObjects];
            }
            streetField.text = @"";
            [streetArr addObjectsFromArray:[areaArr[row] objectForKey:@"list"]];
            [streetpicker reloadAllComponents];
...
-(void)selectStreet
{
    ...
    int row = [streetpicker selectedRowInComponent:0];
    if (streetArr.count)
    {
        if ([[streetArr objectAtIndex:row] isKindOfClass:[NSDictionary class]])
        {
            if ([[streetArr[row] objectForKey:@"title"] isKindOfClass:[NSString class]])
            {
                streetField.text = [streetArr[row] objectForKey:@"title"];
                streetID = [[streetArr[row]objectForKey:@"id"] retain];
            }
