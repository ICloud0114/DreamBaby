...
//                    userNameField = cell.contentField;
                    ...
                    userNameField.placeholder = @"请填写真实姓名，用户名不可修改";
    ...
    if (tableView == self.myTableView)
    {
        if (indexPath.section == 1)
        {
            ...
            if (indexPath.row == 5 )
            {
                ...
                [self.myTableView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:5 inSection:1],[NSIndexPath indexPathForRow:6 inSection:1], [NSIndexPath indexPathForRow:9 inSection:1],[NSIndexPath indexPathForRow:7 inSection:1],[NSIndexPath indexPathForRow:8 inSection:1]] withRowAnimation:UITableViewRowAnimationFade];
            }
            else if (indexPath.row == 6 )
            {
                if (self.selectedIndexPath != nil)
                {
                    ...
                    {
                        if (!self.cityId) {
                            UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:nil message:@"请先选择户籍所在地" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                            [alertView show];
                            [alertView release];
                            return;
                        }
                    ...
                    }else
                    {
                        ...
                        self.selectedIndexPath = indexPath;
                        self.selectedIndexPath =  [NSIndexPath indexPathForRow:6 inSection:1];
                        opened = YES;
...
//        self.myTableView.frame = rect;
//    }
    else if (textField == cityField || textField == areaField || textField == streetField)
    {
        CGRect rect = self.myTableView.frame;
        rect.origin.y = -150;
        self.myTableView.frame = rect;
...
- (void)requestForSignUpwithPass:(NSString *)pass andRight:(NSString *)right
 {
     ...
     if (userName.length == 0 ||nickName.length == 0|| password.length == 0 || idCard.length == 0  || mobile.length == 0 || city.length == 0||area.length == 0 || street.length == 0) {
