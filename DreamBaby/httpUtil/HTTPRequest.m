...
+(NSString*)requestForGetWithPramaStr:(NSString*)requestParamStr method:(NSString*)methodStr withURLtpye:(int)i
{
    ...
    if (i==0)
    {
        requestStr = [NSString stringWithFormat:@"http://192.168.0.155:8004/api/GetFetationInfo.ashx?Method=%@&Params=%@&Sign=%@",methodStr,paramsJson,signs];
    }else if(i==1){
   requestStr = [NSString stringWithFormat:BABYAPI,methodStr,paramsJson,signs];
    }
...
+(NSString*)requestForPostWithPramas:(NSDictionary*)requestParams method:(NSString*)methodStr img:(NSString*)img
{
    ...
    signs = [[FBEncryptorDES md5:signs] uppercaseString];
    //
    NSString *postUrl = [NSString stringWithFormat:BABYAPI,methodStr,paramsJson,signs];
...
//    http://113.105.159.107:84
//    icbcapp.intsun.com
    NSString *requestStr = [NSString stringWithFormat:BABYAPI,methodStr,paramsJson,signs];
    NSLog(@"request %@",requestStr);
    NSString *json = [HTTPRequest requestForGet:requestStr];
//    NSLog(@"jsondes is %@",json);
    //解密
