...
- (void)setImageWithURL:(NSURL *)url forState:(NSString*)state{
    [self setImageWithURL:url placeholderImage:nil options:0 forState:state];
}
...
- (void)setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options forState:(NSString*)state
{
...
//    [manager cancelForDelegate:self];
    ...
    if (url)
    {
        NSDictionary *info = [NSDictionary dictionaryWithObject:state forKey:@"type"];
...
- (void)webImageManager:(SDWebImageManager *)imageManager didProgressWithPartialImage:(UIImage *)image forURL:(NSURL *)url userInfo:(NSDictionary *)info
{
    ...
    else if ([[info valueForKey:@"type"] isEqualToString:@"UIControlStateNormal"])
    {
        [self setImage:image forState:UIControlStateNormal];
    }
    else if ([[info valueForKey:@"type"] isEqualToString:@"UIControlStateSelected"])
    {
        [self setImage:image forState:UIControlStateSelected];
    }
    else if ([[info valueForKey:@"type"] isEqualToString:@"UIControlStateHighlighted"])
    {
